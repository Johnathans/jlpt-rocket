...
import { BookOpen, FileText, MessageSquare, Flame, TrendingUp, Target, ChevronRight } from 'lucide-react';
...
export default function RoadmapPage() {
  const { currentLevel, setLevel } = useJLPTLevel();
  ...
  useEffect(() => {
    const loadStats = async () => {
      try {
        ...
        const allProgress = ReviewSystem.getAllProgress();
        
        // Calculate mastered and learning items
        const kanjiProgress = allProgress.filter(p => p.type === 'kanji');
        const vocabProgress = allProgress.filter(p => p.type === 'vocabulary');
        const sentenceProgress = allProgress.filter(p => p.type === 'sentences');
        
        const getMastered = (items: any[]) => items.filter(i => i.masteryLevel >= 100).length;
        const getLearning = (items: any[]) => items.filter(i => i.masteryLevel > 0 && i.masteryLevel < 100).length;
