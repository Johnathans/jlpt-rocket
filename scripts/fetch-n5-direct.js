...
// Use the official Jisho API to search for JLPT N5 words
async function searchJLPTN5Words() {
  const results = []
  const seenWords = new Set()
  
  // Search for words with JLPT N5 tag
  const jlptSearches = [
    '#jlpt-n5',
    '#jlpt-n4', // Some N4 words might be basic enough
    'jlpt n5',
    'beginner'
  ]
  ...
  for (const searchTerm of jlptSearches) {
    try {
      console.log(`Searching: ${searchTerm}`)
      
      const response = await searchJisho(searchTerm)
      ...
      if (data.data && data.data.length > 0) {
        for (const item of data.data) {
          // Check if it has JLPT N5 tag specifically
          const hasN5Tag = item.tags?.some(tag => 
            tag.toLowerCase().includes('jlpt n5') || 
            tag.toLowerCase().includes('jlpt-n5')
          )
          
          // For N4 searches, only take words that are very common
          const isBasicN4 = searchTerm.includes('n4') ? item.is_common : true
          
          if ((hasN5Tag || (searchTerm === 'beginner' && item.is_common)) && isBasicN4) {
...
// Search for specific common N5 words
async function searchSpecificN5Words() {
  ...
  for (let i = 0; i < commonN5Words.length; i++) {
    ...
    try {
      ...
      if (!response.ok) {
        console.log(`  ❌ HTTP ${response.status} for: ${word}`)
        continue
      }
...
async function saveToFile(data, filename = 'n5-vocabulary-direct.json') {
  try {
...
async function main() {
  ...
  // Save to JSON file
  await saveToFile(finalWords, 'n5-vocabulary-direct.json')
  
  // Try to save to Supabase
  const supabaseSuccess = await saveToSupabase(finalWords)
  
  console.log('\n✅ N5 vocabulary collection completed!')
  console.log(`📁 Data saved to: n5-vocabulary-direct.json`)
  if (supabaseSuccess) {
