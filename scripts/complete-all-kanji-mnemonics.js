// Complete mnemonics for ALL JLPT levels - N5 through N1
const fs = require('fs');
...
// Generate mnemonics for each level
function generateMnemonicsForLevel(level) {
  const levelKanji = kanjiData.filter(k => k.jlpt_level === level);
  
  return levelKanji.map(k => ({
    character: k.character,
    meaning: k.meaning,
    stroke_count: k.stroke_count,
    jlpt_level: k.jlpt_level,
    mnemonic: n5Mnemonics[k.character] || `A memorable way to remember ${k.character} (${k.meaning}): Think of the character's shape and meaning together to create your own visual story!`
  }));
}
...
levels.forEach(level => {
  const mnemonics = generateMnemonicsForLevel(level);
  const outputPath = path.join(__dirname, `../public/data/${level.toLowerCase()}-kanji-mnemonics.json`);
  fs.writeFileSync(outputPath, JSON.stringify(mnemonics, null, 2), 'utf8');
  
  const withCustom = mnemonics.filter(m => !m.mnemonic.includes('create your own')).length;
  const generic = mnemonics.length - withCustom;
  
  console.log(`✅ ${level}: ${mnemonics.length} kanji (${withCustom} custom, ${generic} generic)`);
});

console.log('\n📁 All mnemonic files created in /public/data/');
console.log('\n💡 Note: N5 has custom mnemonics. Other levels have generic placeholders.');
console.log('   You can add custom mnemonics for N4-N1 later using the same pattern!');
