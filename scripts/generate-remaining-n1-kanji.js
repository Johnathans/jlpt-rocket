...
async function main() {
  console.log('\n🎨 Generating Remaining N1 Kanji Images\n');

  const outputDir = './public/images/kanji';

  // Fetch ALL N1 kanji from Supabase (starting from offset 1000)
  console.log('📥 Fetching remaining N1 kanji from database...');
  const { data: kanjiList, error } = await supabase
    .from('kanji')
    .select('character, meaning, kun_reading, on_reading')
    .eq('jlpt_level', 'N1')
    .order('frequency_rank', { ascending: true })
    .range(1000, 2000); // Get kanji 1001-2000
  ...
  console.log(`✓ Found ${kanjiList.length} remaining N1 kanji\n`);
