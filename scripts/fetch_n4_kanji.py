#!/usr/bin/env python3
"""
Fetch N4 Kanji details from jisho.org using jisho-api
Uses our existing kanji.json to get the N4 kanji list
"""
...
def get_n4_kanji_from_our_data():
    """Load N4 kanji characters from our existing kanji.json"""
    with open('public/data/kanji.json', 'r', encoding='utf-8') as f:
        kanji_data = json.load(f)
    
    n4_kanji = [k['character'] for k in kanji_data if k.get('jlpt_level') == 'N4']
    return n4_kanji
...
def main():
    # Get N4 kanji from our existing data
    N4_KANJI = get_n4_kanji_from_our_data()
    
    print(f"Fetching {len(N4_KANJI)} N4 kanji from jisho.org...")
    
    kanji_list = []
    
    for i, kanji in enumerate(N4_KANJI):
        print(f"[{i+1}/{len(N4_KANJI)}] Fetching: {kanji}")
    ...
    # Save to JSON file
    output_file = "jisho_n4_kanji.json"
