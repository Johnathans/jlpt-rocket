#!/usr/bin/env python3
"""
Fetch N1 Kanji details from jisho.org using jisho-api
Uses our existing kanji.json to get the N1 kanji list
"""
...
def get_n1_kanji_from_our_data():
    """Load N1 kanji characters from our existing kanji.json"""
    with open('public/data/kanji.json', 'r', encoding='utf-8') as f:
        kanji_data = json.load(f)
    
    n1_kanji = [k['character'] for k in kanji_data if k.get('jlpt_level') == 'N1']
    return n1_kanji
...
def main():
    # Get N1 kanji from our existing data
    N1_KANJI = get_n1_kanji_from_our_data()
    
    print(f"Fetching {len(N1_KANJI)} N1 kanji from jisho.org...")
    print("This will take approximately 10 minutes...")
    
    kanji_list = []
    
    for i, kanji in enumerate(N1_KANJI):
        print(f"[{i+1}/{len(N1_KANJI)}] Fetching: {kanji}")
    ...
    # Save to JSON file
    output_file = "jisho_n1_kanji.json"
