...
async function processKanjiData() {
  try {
    console.log('Starting kanji data processing...');
    ...
    for (const [character, data] of Object.entries(kanjiData)) {
      ...
      // Get primary meaning (first one) and all meanings
      const primaryMeaning = data.meanings && data.meanings.length > 0 ? data.meanings[0] : '';
      const allMeanings = data.meanings || [];
      
      // Clean WaniKani readings (remove ^ and ! prefixes)
      const cleanWkReadingsOn = data.wk_readings_on ? 
        data.wk_readings_on.map(reading => reading.replace(/^[^!]/, '')) : null;
      const cleanWkReadingsKun = data.wk_readings_kun ? 
        data.wk_readings_kun.map(reading => reading.replace(/^[^!]/, '')) : null;
      ...
      const kanjiEntry = {
        ...
        grade: data.grade,
        radical: '', // Will need to map this separately if needed
        wk_level: data.wk_level,
        wk_meanings: data.wk_meanings,
        wk_readings_on: cleanWkReadingsOn,
        wk_readings_kun: cleanWkReadingsKun,
